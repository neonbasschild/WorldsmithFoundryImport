...
- Accepts a single creature object or an array of creatures.
...
3. Select one or more Worldsmith `.json` files and/or paste JSON into the text
   area.
4. Optionally choose a destination folder and whether to open the sheet(s).
5. Click **Import**.
...
// Import directly from a JSON string (single object or array)
await WorldsmithImport.importFromText(jsonString, { folderId, renderSheet: true });

// Convert without creating (returns { actorData, warnings })
const { actorData } = WorldsmithImport.convertWorldsmith(parsedJson);
```

## Examples

Two sample Worldsmith exports are included under [`examples/`](examples/):

- `stormwing-thunder-griffin.json` — a CR 8 companion mount.
- `eldritch-behemoth.json` — a CR 25 legendary aberration with loot.

## Development

The conversion logic in `scripts/converter.mjs`, `scripts/parsers.mjs`, and
